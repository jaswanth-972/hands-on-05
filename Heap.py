...
class Heap:
    def __init__(self, arr: List[T] = None, min_heap: bool = True) -> None:
        """
        Initializes the heap with the given array.
        
        :param arr: The initial array to build the heap from (default is empty).
        :param min_heap: If True, build a min heap; if False, build a max heap.
        """
        self.heap: List[T] = arr if arr is not None else []
...
if __name__ == '__main__':
    ...
    min_heap.insert(2)
    print("Min Heap after inserting 2:", min_heap)

    root = min_heap.peek()
    print(f'Min Heap root element (peek): {root}')

    popped_value = min_heap.extract()
    print(f'Min Heap after popping root ({popped_value}): {min_heap}')
